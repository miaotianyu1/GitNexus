...
function extractOwnerName(node: SyntaxNode): string | undefined {
...
function collectMethodNodes(ownerNode: SyntaxNode): SyntaxNode[] {
  ...
  // class_interface / protocol_declaration expose method_declaration directly as children.
  for (const child of ownerNode.namedChildren ?? []) {
...
function unwrapMethodTypeText(methodTypeNode: SyntaxNode | null | undefined): string | null {
...
function extractReturnType(methodNode: SyntaxNode): string | null {
  const methodType = methodNode.namedChildren?.find((c) => c.type === 'method_type') ?? null;
  return unwrapMethodTypeText(methodType);
}

function extractParameters(methodNode: SyntaxNode): ParameterInfo[] {
  const params: ParameterInfo[] = [];
  for (const child of methodNode.namedChildren ?? []) {
    if (child.type !== 'method_parameter') continue;
    const methodType = child.namedChildren?.find((c) => c.type === 'method_type') ?? null;
    const nameNode = child.namedChildren?.find((c) => c.type === 'identifier') ?? null;
...
function resolveSelectorName(methodNode: SyntaxNode): string | null {
...
function buildMethodInfo(
  ...
  const name = resolveSelectorName(methodNode);
  if (!name) return null;
...
export function createObjectiveCMethodExtractor(): MethodExtractor {
  return {
